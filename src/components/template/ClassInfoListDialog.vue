...
export default {
  ...
  methods: {
    selectItem(index) {
      ...
      this.currentPlan.trainingPeriods[this.periodIndex].trainingPeriodDays[this.dayIndex].classInfo=this.classInfoList[index]
      this.currentPlan.trainingPeriods[this.periodIndex].trainingPeriodDays[this.dayIndex].classId=this.classInfoList[index].classId
      this.currentPlan.trainingPeriods[this.periodIndex].trainingPeriodDays[this.dayIndex].isRest=0

      let classCount=0;
      this.currentPlan.trainingPeriods[this.periodIndex].trainingPeriodDays.forEach((day,dayIndex)=>{
        if(day.classId){
          classCount++;
        }
      })
      this.currentPlan.trainingPeriods[this.periodIndex].trainingDays=classCount
